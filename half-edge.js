...
export class HalfEdgeDS {
  ...
  getVBOs() {
    ...
    return [coords, this.scalars, normals, indices];
  }

  estrela(vID) {
    const value = this.halfEdges.find(x => x.vertex.vid == vID)
     let next = value.next;
     this.scalars.push(value.vertex.vid)
     while(!this.scalars.includes(next.vertex.vid)){
       this.scalars.push(next.vertex.vid)
       next = next.next;
     }
  }
}